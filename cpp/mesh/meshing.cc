...
System<2, 2> *System<2, 2>::Factory::create(double area) {
  ...
  std::vector<double> pointlist = system_input_mesh->getPointList();
  ...
  const double *pntlst = triout.pointlist;
  {
    ...
    // corresponding boundary classes, if it is not an interior vertex (boundary
    // class 0).
    ...
    // corners in the front and to the back. Move the corresponding second point
    // of the edge containing a corner to the second and second last position.
    // Then sort the remaining vertices topologically to form a connected
    // boundary line.
    ...
    // removed them, and therefore results in missing boundary classes, because
    // the missing boundary class is duplicated with another boundary class. In
    // order to
    for (size_t ibnd = 0; ibnd < bnd2node.size(); ++ibnd) {
      ...
      for (size_t jbnd = 0; jbnd < bnd2node.size(); ++jbnd) {
        ...
        for (int nodeid : nodes) {
          ...
          if (fabs((edge->getPoint(0) - node).norm()) <
                  std::numeric_limits<double>::epsilon() ||
              fabs((edge->getPoint(1) - node).norm()) <
                  std::numeric_limits<double>::epsilon())
