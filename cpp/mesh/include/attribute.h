...
class AttributeExtent {
 public:
  ...
  explicit AttributeExtent(std::size_t size) { sizes.push_back(size); }
  ...
  AttributeExtent &operator()() { return operator()(1); }
  ...
  std::size_t getDimension() const noexcept { return sizes.size(); }
...
enum StorageLocation { VERTEX, EDGE, FACE, CELL, SEGMENT };
