...
Segment<Dim, TopDim> *
System<Dim, TopDim>::getOrCreateSegment(const std::string &name) {
  ...
  segments.emplace_back(std::make_unique<Segment<Dim, TopDim>>(
      mesh(), segments.size() + interfaces.size(), name));
...
template <uint Dim, uint TopDim> System<Dim, TopDim>::Factory::Factory() {
...
template <uint Dim, uint TopDim>
Mesh<Dim, TopDim - 1> *
System<Dim, TopDim>::Factory::segment(const std::string &name) {
  Segment<Dim, TopDim> *seg = system->getOrCreateSegment(name);
  const ID seg_id = seg->getID();
  if (segment_input_meshes.size() <= seg->getID() ||
      segment_input_meshes[seg_id] == nullptr)
    segment_input_meshes.emplace(
        segment_input_meshes.begin() + seg_id,
        std::make_unique<Mesh<Dim, TopDim - 1>>(system_input_mesh.get()));
  return segment_input_meshes[seg_id].get();
}

template <uint Dim, uint TopDim>
// unique_ptr<System<Dim, TopDim>> System<Dim, TopDim>::Factory::create(double
// area)
